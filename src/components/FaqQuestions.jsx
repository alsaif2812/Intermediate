                                        ...
                                        {Object.keys(FAQCardData).map((key) => (
                                                  <div key={key} className="grid grid-cols-12  md:p-0 py-5 ">
                                                            <div className="col-span-12 md:col-span-4 md:flex  md:px-10 items-start justify-end ">
                                                                      <p className='text-3xl font-semibold font-sans text-gray-900 mb-3 md:mb-0'>{key}</p>
                                                            </div>
                                                            <div className="col-span-12 md:col-span-6 ">
                                                                      {FAQCardData[key].map((item, index) => (
                                                                                <div key={item.title}>
                                                                                          <Accordion open={open.group === key && open.index === index} className=' md:mb-12'>
                                                                                                    <AccordionHeader onClick={() => handleOpen(key, index)} className="capitalize bg-blue-100 bg-opacity-50 flex flex-col items-start gap-1 px-3 md:px-4 rounded-md">
