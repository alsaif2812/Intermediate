...
function VideoTextCard({ data, bgColor, borderColor, btnColor }) {
  ...
  return (
    <div id="hero" className={`${bgColor} text-[#3d3d3d] max-h-[1000rem] md:flex md:items-center md:justify-center md:mt-3 md:mb-0 mt-28 py-10 px-5 rounded-3xl md:max-w-[1000rem] md:px-8`}>
          ...
          <div id="tab" className="flex items-center justify-around md:justify-start md:gap-10 text-sm font-bold gap-5 overflow-x-auto">
            {tabs.map((tab) => (
              <h1
                ...
                className={`${selectedTab === tab ? borderColor : ''} hover:border-b-4 transition duration-500 ${borderColor} cursor-pointer`}
              >
                {tab}
              </h1>
            ))}
