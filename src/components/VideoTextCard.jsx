...
function VideoTextCard({ data, bgColor, borderColor, btnColor, txtcolor }) {
  const tabs = Object.keys(data);
  const [selectedTab, setSelectedTab] = useState(tabs[0]);

  return (
    <div id="Videotextcard" className={`${bgColor} text-[#3d3d3d]  h-auto  md:mt-3 md:mb-0 mt-28 py-10 px-5 rounded-3xl  md:px-8`}>
      {data[selectedTab] && data[selectedTab].top ? (
        <p className={`text-2xl ${txtcolor}  font-extrabold`}>{data[selectedTab].top}</p>
      ) : ''}
      ...
      <div className="md:mt-[0.6vw] md:flex w-full">
        <div className="md:w-[60%] mt-10 w-[100%]">
          ...
          >
            {data[selectedTab] && data[selectedTab].body && (
